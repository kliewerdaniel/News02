...
def generate_broadcast_with_llm(summaries: List[Dict[str, Any]], 
    ...
    messages = [
        {
            "role": "system",
            "content": "You are a professional news anchor. Create coherent, flowing news broadcasts that inform and engage listeners."
        },
        {
            "role": "user",
            "content": (
...
def save_digest(digest_text: str, output_dir: str = None, summaries: List[Dict[str, Any]] = None) -> str:
    ...
    # Build sources section if summaries provided
    sources_section = ""
    if summaries:
        sources_section = "\n\n## Article Sources\n\n"
    ...
    metadata = f"""# News Digest - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
